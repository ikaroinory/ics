...
from torch.nn.functional import binary_cross_entropy, cross_entropy, l1_loss, mse_loss
    ...
    def forward(self, backcast):
        ...
        for stack_id in range(len(self.stacks)):
            for block_id in range(len(self.stacks[stack_id])):
                ...
                backcast = backcast.to() - b
                forecast = forecast.to() + f
...
class Block(nn.Module):
    ...
    def __init__(
        self, units, thetas_dim, backcast_length=10, forecast_length=5, share_thetas=False,
        nb_harmonics=None
    ):
